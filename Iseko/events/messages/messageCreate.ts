...
export default {  
  ...
  async run({ 0: message, client }: { 0: Message, client: IsekoClient }) {
  //const prefix = prefixes.find(p => message.content.toLowerCase().startsWith(p));
    ...
    if (message.content == prefix && message.author.id != '787263928408342548') {
      const embed = await client.components.embeds.get('updateEmbed_data') as MessageEmbed
		  embed.author && (embed.author.name = client.user.tag)
    ...
    if (message.content.startsWith(prefix) && message.author.id == '787263928408342548') {
      ...
      if (command.args && args.length < 2) {
        const errEmbed = client.components.embeds.get('errEmbed') as MessageEmbed
      ...
      if (command.guildOnly && message.channel.type == 'DM') return message.reply('I can\'t run this in DMs.')
